...
- `{CODEX_PLAN}`: Plan generated by GPT-5.2
...
6. **IMPORTANT**: Use Task tool to spawn 3 planning agents in PARALLEL:
   - Agent 1: GPT-5.2 (high reasoning with thinking mode)
...
```
You are an implementation planning specialist using GPT-5.2. Create a detailed implementation plan for this feature:
...
codex exec -m gpt-5.2 -c model_reasoning_effort="high" -c thinking="enabled" --skip-git-repo-check "Create a comprehensive implementation plan for: {FEATURE_DESCRIPTION}. Return the plan as markdown text with the following structure:
...
## Next Steps
1. 📋 Review the synthesized plan: `cat plans/{PLAN_FILENAME}`
2. 📚 Check referenced docs: {aidocs_paths}
3. 🔍 Refine requirements if needed
4. 🚀 Begin implementation: `/build plans/{PLAN_FILENAME}`

---
📁 **Full synthesized plan**: `plans/{PLAN_FILENAME}`
...
- "Add payment processing" � `add-payment-processing.md`
- "Fix login bug" � `fix-login-bug.md`
- "Refactor API endpoints" � `refactor-api-endpoints.md`
...
- ✅ Use Glob to check if `plans/` exists BEFORE creating
- ✅ Only create `plans/` if Glob returns no results
- ✅ Create `plans/context/` subdirectory for agent collaboration files
- ✅ Check for `aidocs/` and scan for relevant documentation
- ❌ Never create `plans/` if it already exists (causes errors)
- 🚫 NEVER allow agents to create collaboration files in root directory - ALWAYS use /plans/context

**Multi-Agent Planning**:
- ✅ Spawn all 3 agents in PARALLEL using single Task message
- ✅ Each agent returns plan TEXT (not saved to file)
- ✅ Wait for ALL agents before synthesis
- ✅ Main agent synthesizes final plan
- ✅ Include agent attribution metadata
- ❌ Never skip an agent - run all 3
- ❌ Never let agents save directly to files
- ❌ Never skip synthesis step

**Documentation Integration**:
- ✅ Check aidocs/ for relevant files based on feature keywords
- ✅ Include aidocs paths in agent prompts if found
- ✅ Reference specific docs in synthesized plan
- ✅ Note if no docs found (suggest running /scout with scale 5+)

**File Management**:
- ✅ Use Write tool ONLY for final synthesized plan
- ✅ Include synthesis metadata at top of plan file
- ✅ Use kebab-case for filenames
- ❌ Never write intermediate agent plans to files
